...
export const parseExpense = (text, source = 'UNKNOWN', receivedDate = new Date()) => {
  ...
  if (source === 'NOTIFICATION') {
    // e.g. "Paid ₹450 to Swiggy"
    const paidMatch = cleanText.match(/Paid\s*(?:Rs\.?|INR|₹)\s*([\d.]+)\s*to\s*(.+)/i);
    if (paidMatch) {
      amount = parseFloat(paidMatch[1]);
      merchant = paidMatch[2].trim();
      return { id: uuidv4(), amount, merchant, account: 'UPI', date: receivedDate.toISOString() };
    }

    // e.g. "Sent ₹500 to Rahul"
    const sentMatch = cleanText.match(/Sent\s*(?:Rs\.?|INR|₹)\s*([\d.]+)\s*to\s*(.+)/i);
    if (sentMatch) {
    ...
    // e.g. "Spent ₹500 at Starbucks"
    const spentMatch = cleanText.match(/Spent\s*(?:Rs\.?|INR|₹)\s*([\d.]+)\s*(?:at|on)\s*(.+)/i);
    if (spentMatch) {
      amount = parseFloat(spentMatch[1]);
      merchant = spentMatch[2].trim();
      return { id: uuidv4(), amount, merchant, account: 'Card/UPI', date: receivedDate.toISOString() };
    }
  }

  // 2. Check for SMS Formats (Bank Texts)
  // AU Bank
  if (source.includes('AUBANK') || cleanText.includes('AUBANK')) {
    const auMatch = cleanText.match(/Rs\.?\s*([\d.]+)\s*is debited.*(?:to|at)\s*(.+?)(?:\s*on|\s*Ref|\s*\.|$)/i);
    if (auMatch) {
  ...
  if (!amount && cleanText.match(/debited|spent|paid/i)) {
    const genericAmountMatch = cleanText.match(/(?:Rs\.?|INR|₹)\s*([\d.]+)/i);
